...
vec2 bezier(vec2 a, vec2 b, vec2 c, vec2 d, float f) {
    ...
    return mix(mix(mix(a, b, f), mix(b, c, f), f), mix(mix(b, c, f), mix(c, d, f), f), f);
}
...
void main()
{
    ...
    vec2 e = warpPSpaceToESpace(splinePoints[0], splinePoints[1], splinePoints[2], splinePoints[3], p, scale / u_heightOfTile);
    vec2 z = bezier(splinePoints[0], splinePoints[1], splinePoints[2], splinePoints[3], e.x);
    float d = length(p - z);

    for (int i = 1; i < beziersInSpline; ++i) {
        // splinePointIndex
        int j = 3 * i;
        vec2 ec = warpPSpaceToESpace(splinePoints[j], splinePoints[j + 1], splinePoints[j + 2], splinePoints[j + 3], p, scale / u_heightOfTile);
        vec2 zc = bezier(splinePoints[j], splinePoints[j + 1], splinePoints[j + 2], splinePoints[j + 3], ec.x);
        float dc = length(p - zc);
        if (dc < d) {
            e = ec;
            z = zc;
            d = dc;
    ...
    float splineLengthEstimate = 0.0;
    for (int i = 1; i < numberOfGuidePoints; ++i)
    splineLengthEstimate += length(splinePoints[i - 1] - splinePoints[i]);
    float repeats = u_repeats >= 0.0 ? u_repeats : ceil(splineLengthEstimate / (u_heightOfTile / scale));
    ...
    // if (p.x < float(numberOfGuidePoints) / 20.0) gl_FragColor *= 0.0;
